                    ...
                    <div class="col-12 ">
                        <div class="card fullcalendar-card">
                            ...
                            <div class="card-block">
                                <div class="row">
                                    <div class="col-12">
                                        <div class="col-12 btn-newStudyHR mb-3">
                                            <button id="addOutTime" type="button" class="btn btn-rounded btn-success">
                                                <span>Agregar tiempo fuera</span>
                                            </button>
                                        </div>
                                        <div class="col-md-4 m-b-30">
                                            <input type="text" id="date" class="form-control" placeholder="Fecha">
                                        </div>
                                        ...
                                        <div id='external-events' class="external-events">
                                            @foreach ($studies as $study)
                                                ...
                                                @if ($study->status == 'Pagado')
                                                    <div id="{{$letter}}{{sprintf('%06d',$study->folio)}}" duration="{{$study->duration}}" class='fc-event {{$letter}}{{sprintf('%06d',$study->folio)}}'>
                                                        {{$letter}}{{sprintf('%06d',$study->folio)}}
                                                        <br>{{$study->patient_name}} {{$study->paternal_surname}} {{$study->maternal_surname}}<br>
                                                        @if (isset($study->appointment))
                                                            Cita: {{$weekMap[strftime("%w",strtotime($study->appointment->date))]}} {{strftime("%d",strtotime($study->appointment->date))}} {{strtoupper($months[strftime("%m",strtotime($study->appointment->date))])}} {{strftime("%Y",strtotime($study->appointment->date))}}<br>{{$study->appointment->time}}
                                                        @else
                                                            Sin agendar cita
                                                        @endif
                                                    </div>
...
<script src="https://unpkg.com/sweetalert/dist/sweetalert.min.js"></script>
    ...
    <script type="text/javascript">
        ...
        $(window).on('load', function() {
                    ...
                    success: function (array) {
                        ...
                        for (const [key, value] of Object.entries(array['studies'])) {
                            ...
                            if(value['appointment'] != null){
                                var cita = 'Cita:'+value['appointment'].date+'\n'+value['appointment'].time;
                            }else{
                        ...
                        calendar = $('#calendar').fullCalendar({
                            ...
                            drop: function(date) {
                                var object = $(this);
                                //Id del evento (Folio o tipo de tiempo fuera)
                                var id = $(this).attr('id');
                                if(id == undefined){
                                    id= $(this).text();
                                }
                                if(!id.startsWith("D") && !id.startsWith("R")){
                                    id = $("#inputType").val();
                                }
                                var day = date.day(); // number 0-6 with Sunday as 0 and Saturday as 6
                                //Id del radiologo al que se le asigna el evento
                                var idRadiologo = radiologists[day]['id'];
                                //Fecha en que se asigna
                                var today = todayJS;
                                //Hora en que se asigna  
                                var hours = date.format("HH:mm");
                                //Duracion del evento
                                var durationEvent = $(this).attr('duration');
                                if(durationEvent == ""){
                                    durationEvent = '00:15'
                                }
                                //var horasEnd = addTimes(horas,durationEvent);
                                ...
                                alert(day);
                                alert("horas:"+hours);
                                //alert("horas Fin:"+horasEnd);
                                alert(radiologists[day]['id'])
                                alert(radiologists[day]['name'])
                                alert('{{$today}}')    
                                */           
                                console.log("date " + date);
                                $.ajax({
                                    type: "POST",
                                    url: "{{ url('/newEventDrop') }}",
                                    data: {_token : '{{ csrf_token() }}',"id": id,"idRadiologo": idRadiologo,"today": today,"hours": hours,"durationEvent": durationEvent },
                                    success: function (flag) {
                                        object.remove(); 
                                        //var nueveo = $("."+id).attr("id", 18);
                                        //console.log("nueveo" + nueveo.attr("id"));
                            ...
                            eventDrop: function(event, delta, revertFunc) {
                                ...
                                var id = event.id;
                                //alert(id);
                                var day = event.start.format("d"); // number 0-6 with Sunday as 0 and Saturday as 6
                                //Id del radiologo al que se le asigna el evento
                                var idRadiologo = radiologists[day]['id'];
                                //Fecha en que se asigna
                                var today = todayJS;
                                //Hora en que se asigna  
                                var hours = event.start.format("HH:mm");
                                //Hora en que termina el evento
                                var hoursEnd = event.end.format("HH:mm");
                                //Duracion del evento
                                var durationEvent = minTimes(hoursEnd,hours);                    
                                /*
                                var day = event.start.format("d")
                                var horas = event.start.format("HH:mm")
                                var horasEnd = event.end.format("HH:mm")
                                alert(day);
                                alert("horas:"+horas);
                                alert("horas Fin:"+horasEnd);
                                alert(radiologists[day]['id'])
                                ...
                                */
                                ...
                                /*for (var key in radioStudy) {
                                    //console.log("key " + key + " has value " + radioStudy[key]);
                                    //console.log("eventDrop-radioStudy " + radioStudy[key]);
                                    $("#"+key).attr("color",radiologists[radioStudy[key]]);
                                }*/
                            /*
                                alert($("#"+id).attr('id'));
                                $("#"+id).addClass("red");

                                console.log('event', event.title.split("\n")[0]);
            ...
            /*
            $( ".radiologoSelect" ).change(function() {
                id = $(this).attr('idStudy');
                radioStudy[id] = $( this ).val();
                //console.log("change ",radioStudy);
                $("."+id).attr("color", radiologists[radioStudy[id]]);
            ...
            */
            calendar = $('#calendar').fullCalendar({
                ...
                drop: function(date) {
                    ...
                    */           
                    ...
                    for (var key in radioStudy) {
                        //console.log("key " + key + " has value " + radioStudy[key]);
                        //console.log("drop-radioStudy " + radioStudy[key]);
                        //console.log("drop-radioStudy " + radiologists[radioStudy[key]]);
                        $("."+key).attr("color",radiologists[radioStudy[key]]);
                    }
                    */
            ...
            $( "#addOutTime" ).click(function() {
                ...
                */
                $('#modalOutTime').modal('show');
                /*
                $("#external-events").prepend("<div class='fc-event BlueViolet'>Tiempo muerto</div>");
                $('#external-events .BlueViolet').each(function() {
                    $(this).data('event', {
                        title: $.trim($(this).text()),
                        duration: '00:15',
                        stick: false,
            ...
            $( "#addEvent" ).click(function() {
                var durationOver = $("#durationOver").val();
                var inputType = $("#inputType").val(); 
                if(inputType != null){             
                    $("#external-events").prepend("<div id="+idLast+" class='fc-event BlueViolet' duration="+durationOver+" >"+inputType+"</div>");
                    $('#external-events .BlueViolet').each(function() {
                        $(this).data('event', {
                            title: $.trim($(this).text()),
                    ...
                    idLast++;
                    $("#modalOutTime").modal('hide');

                }
                /*
                var myEvent = {
                title:"Tiempo muerto\n"+inputType,
                start: '{{$today}}T'+startOver+':00',
                end: '{{$today}}T'+finishOver+':00',
                className: 'BlueViolet'
                };
                calendar.fullCalendar( 'renderEvent', myEvent );
                */
    ...
    channel.bind('App\\Events\\buyEvent', function(data) {
        var study = data.message.study;
        var arrayStudies = data.message.arrayStudies;
        ...
        if(study.appointment != null){
            var cita = 'Cita:'+study.appointment.date+'<br>'+study.appointment.time;
        }else{
            var cita = 'Sin agendar cita';
        }

        if(study.duration != null){
            var duration = study.duration;
        }else{
            var duration = '';
        }
        var Main = $('<div id="modal'+letter+padWithLeadingZeros(study.folio, 6)+'" class="modal fade '+study.status+'" tabindex="-1" role="dialog" aria-labelledby="modal'+study.id+'Title" aria-hidden="true"></div>');
        var Second = $('<div class="modal-dialog modal-dialog-centered" role="document"></div>');
        var Third = $('<div class="modal-content"></div>');
        var FourthH = $('<div class="modal-header"></div>');
        $(FourthH).append('<h5 class="modal-title" id="modal'+study.id+'Title">Datos del estudio:</h5>');
        $(FourthH).append('<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>');
        var FourthB = $('<div class="modal-body"></div>');
        var Fifth = $('<div class="card card-border-c-blue"></div>');
        var SixthH = $('<div class="card-header d-block"></div>');
        $(SixthH).append('<a href="#!" class="text-secondary">'+letter+padWithLeadingZeros(study.folio, 6)+' - '+study.patient_name+' '+study.paternal_surname+' '+study.maternal_surname+' </a>');
        $(SixthH).append('<br><strong>Cita:</strong>');
        $(SixthH).append(cita);
        var SeventhH1 = $('<div class="row"></div>');
        $(SeventhH1).append('<div class="col-md-6">'+study.patient_email+'<br>'+study.patient_phone+'</div>');
        if (study.birthday != null) {
            $(SeventhH1).append('<div class="col-md-6">'+study.birthday+'<br>Edad: '+data.message.old+' Años</div>');
        }
        if (study.radiologist != null && study.radiologist != "" ) {
            $(SeventhH1).append('<div class="mt-1"><p class="task-due"><strong> Radiólogo: </strong><strong class="label label-primary">'+study.radiologist+'</strong></p></div>');
        }
        var SeventhH2 = $('<div class="row"></div>');
        $(SeventhH2).append('<div class="mt-1 text-center col-12">Doctor</div>');
        var EighthH2 = $('<div class="text-justify"></div>');
        if (study.doctor_id == 0) {
            $(EighthH2).append('<div class="col-12">'+study.doctor_name+'</div>');
        }else{
            $(EighthH2).append('<div class="col-12">'+study.doctor.alias+'</div>');
            $(EighthH2).append('<div class="col-12">'+study.doctor.user.name+' '+study.doctor.paternalSurname+' '+study.doctor.maternalSurname+'</div>');
        }
        $(SeventhH2).append(EighthH2);
        $(SixthH).append(SeventhH1);
        $(SixthH).append(SeventhH2);
        ...
        Object.entries(arrayStudies).forEach(itemStudy => {
            const [key, value] = itemStudy;
            console.log(key, value);
            console.log("tipeof: "+typeof value["title"])
            console.log("value: "+ value["title"])
        ...
        $(SeventhB1).append(EighthB1);
        if (study.observations != null) {
            var observations = study.observations;
        }else{
            var observations = "";
        }
        var SeventhB2 = $('<div class="row"><strong>Observaciones adicionales:</strong><div class="col-12">'+observations+'</div></div><hr>');
        ...
        $(SixthB).append(SeventhB3);
        if(study.status != 'Realizado'){
            $(SixthB).append('<button class="btn btn-primary btn-sm finish" type="button" qr="'+study.qr+'" id="button'+letter+padWithLeadingZeros(study.folio, 6)+'"disabled>TERMINAR ESTUDIO</button>');
            var colorClass = '';
            var dropStudy = true;
        }else{
            var colorClass = 'Green';
        ...
        $("#external-events").prepend('<div id="'+letter+padWithLeadingZeros(study.folio, 6)+'" duration="'+duration+'" class="fc-event '+letter+padWithLeadingZeros(study.folio, 6)+'">\
                                            '+letter+padWithLeadingZeros(study.folio, 6)+'\
                                            <br>'+study.patient_name+' '+study.paternal_surname+' '+study.maternal_surname+'<br>'+cita+'</div>');

        $('#'+letter+padWithLeadingZeros(study.folio, 6)).each(function() {
            thisDuration = $(this).attr('duration');
            if(thisDuration == ""){
                thisDuration = '00:15'
            }
